	...
	class pasal_ecommerce_About_Page {
		...
		public function pasal_ecommerce_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap acp-about-wrap epsilon-wrap">';
				...
				if ( ! empty( $welcome_title ) ) {
					...
					echo '<img src="https://codethemes.co/wp-content/uploads/2016/02/codethemes-logo.svg"></h1>';
				}
...
//
		...
		public function getting_started() {
			...
			if ( ! empty( $this->config['getting_started'] ) ) {
				...
				if ( ! empty( $getting_started ) ) {
				    ?>
					<?php echo '<div class="feature-section three-col">';
			...
			if ( ! empty( $recommended_actions ) ) {
				...
				if ( ! empty( $actions ) && is_array( $actions ) ) {
					...
					foreach ( $actions as $action_key => $action_value ) {
						...
						echo '<div class="acp-about-page-action-required-box">';
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							if($network=='local' && $class=='install-now button') {
                                ?>
                                <p><?php echo esc_html__('If plugin installation failed then click on the&nbsp;','pasal-ecommerce'); ?><a
                                            href="<?php echo esc_url(admin_url('themes.php?page=tgmpa-install-plugins')); ?>"
                                            target="_blank"><?php echo esc_html__('link.', 'pasal-ecommerce'); ?></a>
                                </p>
                                <?php
                            }
                                ?>
		...
		public function child_themes() {
			echo '<div id="child-themes" class="acp-about-page-tab-pane">';
				...
				if ( ! empty( $child_themes['content'] ) && is_array( $child_themes['content'] ) ) {
					...
					for ( $i = 0; $i < count( $child_themes['content'] ); $i ++ ) {
						...
						if ( ! empty( $child['image'] ) ) {
							echo '<div class="acp-about-child-theme">';
							echo '<div class="acp-about-page-child-theme-image">';
							echo '<img src="' . esc_url( $child['image'] ) . '" alt="' . ( ! empty( $child['image_alt'] ) ? esc_html( $child['image_alt'] ) : '' ) . '" />';
							if ( ! empty( $child['title'] ) ) {
								echo '<div class="acp-about-page-child-theme-details">';
							...
							echo '</div><!--acp-about-page-child-theme-image-->';
							echo '</div><!--acp-about-child-theme-->';
						}// End if().
					}// End for().
					echo '</div>';
				}// End if().
        ...
        public function demo_import() {
            echo '<div class="feature-section three-col demo-import-col">';
            echo '<div class="feature-section acp-demo-import"><img src="'.esc_url(get_template_directory_uri().'/screenshot.png').'" width="300px"><div class="demo-import-footer"><h4>';
            printf(
                wp_kses(
                /* translators: 1: link to WP admin new post page. */
                    __( 'Need To Import Demo&nbsp;&nbsp;&nbsp;<a href="%1$s" class="button button-primary">Click Here</a>', 'pasal-ecommerce' ),
                    array(
                        'a' => array(
                            'href' => array(),
                            'class' => array(),
        	...
        	echo '<div class="feature-section landing-demo-import"><img src="' . esc_url($demo_contents['image_link']) . '"  width="300px"><div class="demo-import-footer"><h4>';
            echo  esc_html($demo_contents['theme_lebel']);echo '</h4>';
            echo '<a class="button button-primary" href="' . esc_html($demo_contents['demo_link']) . '" target="_blank">' . esc_html($demo_contents['demo_label']) . '</a>';
            echo '<a class="button button-primary" href="' . esc_html($demo_contents['button_link']) . '" target="_blank">' . esc_html($demo_contents['button_label']) . '</a>';
            echo '</div>';
            echo '</div>';
			}
		 }
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<div id="free_pro" class="acp-about-page-tab-pane acp-about-page-fre-pro">';
					echo '<table class="free-pro-table">';
					echo '<thead>';
					echo '<tr class="acp-about-page-text-right">';
					echo '<th></th>';
					...
					if ( ! empty( $free_pro['pro_theme_link'] ) && ! empty( $free_pro['get_pro_theme_label'] ) ) {
						...
						echo '</td>';
						echo '<td colspan="2" class="acp-about-page-text-right"><a href="' . esc_url( $free_pro['pro_theme_link'] ) . '" target="_blank" class="button button-primary button-hero">' . wp_kses_post( $free_pro['get_pro_theme_label'] ) . '</a></td>';
						echo '</tr>';
					}
					echo '</tbody>';
					echo '</table>';
